...
if (isset($_POST['kirim'])) {
	...
	move_uploaded_file($src, "../../assets/image/" . $gal);
	if ($aksi == "qw") {
		$update = $db->cud("UPDATE tbl_galerry SET id_lokasi=?, caption =?, galery = ? WHERE id =?", [$id_lokasi, $caption, $gal, $id]);
		if ($update) {
		...
		$kirim = $db->cud("INSERT into tbl_galerry values(?,?,?,?)", [null, $id_lokasi, $caption, $gal]);
