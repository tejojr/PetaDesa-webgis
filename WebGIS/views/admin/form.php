...
if (isset($_POST['kirim'])) {
	...
	move_uploaded_file($src, "../../assets/image/" . $gbr);
	if ($aksi == "qw") {
		$update = $db->cud("UPDATE tbl_lokasi SET id_marker = ?, nama_lokasi=?,lat=?,lng=?,alamat=?,no_hp=?,foto=?, deskripsi=?, url=? where id =?", [$id_lokasi, $nama_lokasi, $lat, $lng, $alamat, $nohp, $gbr, $deskripsi, $url, $id]);
		if ($update) {
		...
		$kirim = $db->cud("INSERT into tbl_lokasi values(?,?,?,?,?,?,?,?,?,?,?)", [null, $kode, $id_lokasi, $nama_lokasi, $lat, $lng, $alamat, $nohp, $gbr, $deskripsi, $url]);
		if ($kirim) {
