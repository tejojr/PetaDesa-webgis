...
if (isset($_POST['kirim'])) {
	...
	move_uploaded_file($src, "../../assets/icon/" . $ikon);
	if ($aksi == "qw") {
		$update = $db->cud("UPDATE tbl_marker SET nama= ?, gbr=? WHERE id =?", [$kategori, $ikon, $id_mark]);
		if ($update) {
		...
		$kirim = $db->cud("INSERT into tbl_marker values(?,?,?)", [null, $kategori, $ikon]);
